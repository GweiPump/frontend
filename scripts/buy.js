...
deployedGweiPump.methods.wtiUsdPenniesPriceOracle().call((err, wtiPriceOracleResponse) => {
...
deployedGweiPump.methods.getWti40Milliliters().call((err, isPumpFilledResponse) => {
  if(isPumpFilledResponse === undefined){
    ...
    document.getElementById("getWei40mLmatic").innerHTML = Math.round( Number( ( ( (11*isPumpFilledResponse)/10)).toFixed(8) ) );
  }
})
      ...
      params: [
        {
          ...
          value: web3.utils.toHex(Math.round(msgValueScaledMempool)), //Scale up 10% to handle price changes in mempool.
        },
      ],
    })
